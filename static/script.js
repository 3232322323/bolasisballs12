...
function renderAdminUsersList(adminName) {
  db.ref('users').once('value', (snapshot) => {
    ...
    // Add admins list at the bottom
    const adminsBottom = document.createElement("div");
    adminsBottom.className = "admins-bottom-list";
    adminsBottom.innerHTML = `<b>Admins:</b> ${ADMINS.map(a => `<span class="admin-name">${a}</span>`).join(" ")}`;
    listDiv.appendChild(adminsBottom);
  });
}
